...
export default function TodaysDiary() {
  ...
  const handleEndConversation = () => {
    // Logic to generate summary and end conversation
    console.log("日記を終了し、要約を生成します")
  }

  return (
    <Card className="w-full max-w-2xl h-[80vh] flex flex-col bg-[#fffaf0] dark:bg-gray-800 mx-auto">
      <CardHeader className="text-center">
        <CardTitle className="text-2xl font-serif">{date}</CardTitle>
      </CardHeader>
            ...
            >
              <div className={`flex items-end ${message.sender === "user" ? "flex-row-reverse" : "flex-row"}`}>
                <Avatar className="w-8 h-8">
                  <AvatarFallback>{message.sender === "ai" ? "📔" : "✍️"}</AvatarFallback>
                </Avatar>
                <div
      ...
      <CardFooter className="flex flex-col space-y-2">
        ...
        <div className="flex w-full space-x-2">
          ...
          <Button variant="outline" onClick={handleEndConversation} className="flex-1">
            日記を終える
          </Button>
        </div>
      </CardFooter>
